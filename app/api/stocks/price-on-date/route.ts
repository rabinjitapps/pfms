...
import { fetchStockPriceOnDate } from '@/lib/stockPrice';
...
export async function GET(req: NextRequest) {
  ...
  try {
    const result = await fetchStockPriceOnDate(symbol, date);
    if (!result) {
      return NextResponse.json(
        { error: 'No price found for this stock on or before that date' },
        { status: 404 }
      );
    }
