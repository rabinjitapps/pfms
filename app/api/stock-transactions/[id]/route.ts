...
export async function DELETE(
  ...
  if (txnLookupErr) {
    console.error('Failed to look up stock transaction for delete:', txnLookupErr);
  ...
  if (holdingLookupErr) {
    console.error('Failed to look up stock holding for delete:', holdingLookupErr);
  ...
  // negative — i.e. a later SELL already depends on these shares.
  ...
  if (allTxnsErr) {
    console.error('Failed to fetch stock transactions for delete check:', allTxnsErr);
  ...
  if (resultingQuantity < -EPSILON) {
    return NextResponse.json(
      {
        error:
          'Deleting this would leave a SELL with no matching shares — delete the later SELL transaction first.',
      },
      { status: 400 }
    );
  }

  const { error } = await supabaseAdmin.from('stock_transactions').delete().eq('id', id);
