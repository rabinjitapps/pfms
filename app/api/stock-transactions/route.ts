...
export async function POST(req: NextRequest) {
  ...
  // For a SELL, make sure there are actually enough shares to sell —
  // same guard as the mutual fund side, so an over-sell can't silently
  // drive totalQuantity negative.
  if (type === 'SELL') {
    ...
    if (txnFetchErr) {
      console.error('Failed to fetch existing stock transactions:', txnFetchErr);
      ...
      return NextResponse.json(
        {
          error:
            heldQuantity > EPSILON
              ? `You only hold ${heldQuantity.toFixed(4)} shares — cannot sell ${quantityNum.toFixed(4)}.`
              : 'You hold no shares of this stock to sell.',
        },
        { status: 400 }
      );
    }
