...
import { StockHoldingSummary } from '@/types';
...
export default function StockTransactionModal({ holding, onClose, onSaved }: Props) {
  ...
  const [priceValue, setPriceValue] = useState(holding.stock.latest_price?.toString() ?? '');
  ...
  useEffect(() => {
    ...
    const key = `${holding.stock.symbol}:${date}`;
    ...
    (async () => {
      try {
        const res = await fetch(
          `/api/stocks/price-on-date?symbol=${encodeURIComponent(holding.stock.symbol)}&date=${date}`
        );
        ...
        if (data.date !== date) {
          setPriceFetchNote(`Market closed on ${date}; using price from ${data.date}.`);
        } else {
          setPriceFetchNote('');
        }
      } catch {
  ...
  async function handleUpdatePrice() {
    ...
    try {
      const res = await fetch('/api/stocks/update-price', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ stockId: holding.stock.id, price: Number(priceValue), date: priceDate }),
      });
      const data = await res.json();
      if (!res.ok) {
    ...
    <div style={styles.overlay} onClick={onClose}>
      <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
        <div style={styles.header}>
          <div>
            <h2 style={styles.title}>{holding.stock.name}</h2>
            <p style={styles.subtitle}>{holding.stock.symbol}</p>
          </div>
          ...
          <div>
            ...
            {entryMode === 'amount' && computedQuantity !== null && (
              <p style={styles.computedAmount}>
                ≈ {computedQuantity.toLocaleString('en-IN', { maximumFractionDigits: 4 })} shares
              </p>
            )}
        ...
        {tab === 'price' && (
          <div>
            <p style={styles.hint}>
              Manually set the latest price for this stock. This overrides the auto-fetched value
          ...
          <div>
            ...
            {wasAlreadyNegative && (
              <p style={styles.negativeBanner}>
                ⚠ This stock&apos;s quantity has gone negative ({currentQuantity.toFixed(4)}) — one
