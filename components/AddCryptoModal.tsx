...
export default function AddCryptoModal({ onClose, onAdded }: Props) {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      setSearchError('');
      try {
        const res = await fetch(`/api/cryptos/search?q=${encodeURIComponent(query.trim())}`);
  ...
  async function handleAddSelected() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(data.error || 'Could not add crypto.');
      }
    } finally {
      setSaving(false);
    }
    ...
    <div style={styles.overlay} onClick={onClose}>
      <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
        <div style={styles.header}>
          <h2 style={styles.title}>Add a crypto</h2>
        ...
        {mode === 'search' ? (
          <div>
            <input
              style={styles.input}
              type="text"
              placeholder="Search by coin name or symbol, e.g. Bitcoin, ETH, Solana"
              value={query}
              onChange={(e) => {
          ...
          <div>
            ...
            <p style={styles.hint}>
              Use Yahoo Finance pair format: ticker plus fiat currency, e.g. &ldquo;BTC-USD&rdquo;
              for Bitcoin priced in US dollars, or &ldquo;BTC-INR&rdquo; for rupees.
            </p>
            <label style={styles.label}>
              Coin name
              <input
