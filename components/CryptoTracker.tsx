...
import { CryptoPortfolioSummary, CryptoHoldingSummary } from '@/types';
import { xirr, CashFlow } from '@/lib/xirr';
import AddCryptoModal from './AddCryptoModal';
import CryptoTransactionModal from './CryptoTransactionModal';
import AppShell from './AppShell';
import styles from './CryptoTracker.module.css';
...
function earliestTransactionDate(h: CryptoHoldingSummary): string | null {
...
// ISO date strings, plus the raw day count. Mirrors StockTracker's holdingAge.
...
function holdingCashFlows(h: CryptoHoldingSummary, today: string): CashFlow[] {
...
export default function CryptoTracker({ displayName }: { displayName: string }) {
  const [portfolio, setPortfolio] = useState<CryptoPortfolioSummary | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  const [showAddModal, setShowAddModal] = useState(false);
  const [activeHolding, setActiveHolding] = useState<CryptoHoldingSummary | null>(null);
  ...
  async function handleRefreshPrice() {
    ...
    setRefreshMsg('');
    try {
      const res = await fetch('/api/cryptos/refresh-price', { method: 'POST' });
      const data = await res.json();
      if (res.ok) {
        setRefreshMsg(
          data.updated === 0
            ? 'No cryptos to update.'
            : `Updated ${data.updated} of ${data.total} coin${data.total === 1 ? '' : 's'}.`
        );
        await load();
      } else {
  ...
  if (loading) {
    return (
      <AppShell active="crypto" displayName={displayName}>
        <div className={styles.page}>
          <p className={styles.loadingText}>Loading your crypto…</p>
        </div>
      </AppShell>
    );
  }
    ...
    <AppShell active="crypto" displayName={displayName}>
      <div className={styles.page}>
        <header className={styles.pageHeader}>
          ...
          <h2 className={styles.pageTitle}>Crypto</h2>
        </header>
        ...
        <main className={styles.main}>
          ...
          {portfolio && (
            ...
            <div className={styles.summaryGridSecondary}>
              <div>
                <p className={styles.summaryLabel}>Coins held</p>
        ...
        {portfolio && portfolio.holdings.length === 0 ? (
          <div className={styles.emptyState}>
            ...
            <p className={styles.emptyBody}>
              Add your first coin to start tracking quantity, price, and gains.
            </p>
          </div>
        ) : (
          <div className={styles.holdingsList}>
            ...
            {portfolio?.holdings.map((h) => {
              ...
              const cryptoXirrValue = cryptoXirr.get(h.id) ?? null;
                ...
                >
                  <div className={styles.holdingMain}>
                    <p className={styles.holdingName}>
                      {h.crypto.name} <span className={styles.holdingSymbol}>{h.crypto.symbol}</span>
                    </p>
                    {h.totalQuantity < -0.00000001 && (
                      <p className={styles.holdingWarning}>
                        ⚠ Quantity gone negative ({h.totalQuantity.toFixed(8)}) — a sell exceeded
                    ...
                    <p className={styles.holdingMeta}>
                      <span className={styles.holdingMetaEmphasis}>
                        {formatQty(h.totalQuantity)}
                        {' · Avg ₹'}{h.avgPrice.toFixed(2)}
                      </span>
                      {' · '}₹{(h.crypto.latest_price ?? 0).toFixed(2)}
                      {h.crypto.latest_price_date && (
                        <span className={styles.holdingMetaMuted}> as of {h.crypto.latest_price_date}</span>
                      )}
                    </p>
                    {age && (
                  ...
                  <span
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      handleDeleteHolding(h.id, h.crypto.name);
                    }}
                  >
                    Remove
                  </span>
        ...
        {showAddModal && (
          <AddCryptoModal onClose={() => setShowAddModal(false)} onAdded={load} />
        )}
