...
export default function BulkImportModal({ onClose, onImported }: Props) {
  ...
  async function handleUpload() {
    ...
    try {
      ...
      const res = await fetch('/api/expense-entries/bulk', {
        method: 'POST',
        body: formData,
      });
  ...
  return (
    <div style={styles.overlay} onClick={onClose}>
      <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
        ...
        <p style={styles.body}>
          Add many income and expense entries in one go. Download the template below, fill
          it in, then upload it here — heads that don&rsquo;t exist yet are created automatically.
        </p>

        <a href="/api/expense-entries/template" download style={styles.templateLink}>
        ...
        {result && (
          <div style={styles.resultBox}>
            ...
            {result.createdHeads.length > 0 && (
              <p style={styles.resultDetail}>
                New heads created: {result.createdHeads.join(', ')}
              </p>
            )}
